...
def index(request):
    ...
    return render(request,'bikehub.html',context = context_dict)
...
def rent_now(request):
    try:
        if request.method == 'POST':
            ...
            id_list = set()
            for cycles in cycle.objects.filter(availability__exact = 'Available'):
            ...
            for cycle_id in id_list:
                print(Bookings.objects.filter(cycle_id__exact = cycle_id).order_by('id'))
            ...
            for cycles in cycle.objects.filter(category__exact = cat, availability__exact = 'Available'): 
                ...
                # redirect('/')
                messages.success(request,'Please choose another category as this particular category cycles are out of stock !')
            ...
            hours = seconds/3600
            print(hours)
            cost = (hours+1)*math.ceil(float(cat_cost)/24) - disamt*0.01*(hours+1)*(cat_cost/24) + (insamt) + accamt
            if cost <= 0:
                cost = 2
            # print(cost)
        ...
        messages.success(request,'Please Choose all the values!')
        return HttpResponseRedirect('/')
